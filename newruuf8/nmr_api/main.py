...
def _ncd_relevance(discovered_panel: list, present_metabolites: list) -> list:
    """For each Thai NCD, report (a) its own honest diagnostic AUC + biomarker
...
def _run_cohort_pipeline(X, bin_ppm, label_map=None, normalize="pqn",
    ...
    # Metabolites detected in THIS sample (annotation output) — drives the
    # per-NCD match/no-match panel regardless of whether labels are present.
    ...
    if include_biomarkers and label_map and not M.empty:
        ...
        if len(set(label_map[str(s)] for s in rows)) >= 2 and len(rows) >= 8:
            ...
            out["biomarkers"] = disc
    # Live NCD relevance: each NCD's own diagnostic AUC + which of its
    # biomarkers this sample actually contains (match/no-match).
    out["ncd_relevance"] = _ncd_relevance(stable_panel, present_metabolites)
    # Feature selection (Track-1 fn 3): the most informative ppm positions of the
    # ~20k bins, each annotated to its nearest metabolite + NCD relevance. Supervised
    # when a full label vector is present, else unsupervised (signal content).
    try:
        y_arr = None
        if label_map:
            vals = [label_map.get(str(s)) for s in Xn.index]
            if all(v is not None for v in vals) and len(set(vals)) >= 2:
                y_arr = np.array(vals)
        out["diagnostic_ppm"] = spectral_cohort.select_diagnostic_ppm(
            Xn, bin_ppm, y=y_arr, top_k=8)
    except Exception:
        pass
    return out
