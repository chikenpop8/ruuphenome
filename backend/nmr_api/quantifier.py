"""
F8 — GISSMO transformer quantifier (HONEST-NEGATIVE exhibit; disabled by default).
...
**RESULT (2026-07-03): it did NOT work.** It was trained on an H100 and evaluated
on the real BMRB held-out set (`--validate-bmrb`): **F1 0.13 — the worst method**,
and `hybrid+quant` only matched the deterministic+pSCNN `hybrid` by trading
precision for recall. Root cause: it trains on fixed-width single-Gaussian GISSMO
fingerprints (no J-multiplets / real intensities) and does NOT close the sim-to-real
gap. It is kept as a documented honest-negative (validate-first discipline), is
checkpoint-gated OFF, and is NOT wired into the app. The practical Track-1 winner is
deterministic + pSCNN (see `pscnn.py`). RUO; not clinical.
...
def status() -> Dict:
    return {
        ...
        "role": "HONEST-NEGATIVE exhibit — evaluated on real BMRB held-out, F1 0.13 (worst method); disabled, not wired into the app",
    }
